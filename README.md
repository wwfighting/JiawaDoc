...
http://192.168.1.54/test/jvawa/event_rotate.php
...
|:--------:|:---------:|:---------:|
| uid      | String    | 用户id |
| lid      | String    | 礼券id 只有action为"2"的时候才传|
| action   | String    | 请求行为("0"：判断该用户用没有机会抽奖 "1"：得到所有的礼品券 "2"：将抽到的礼品券插入到数据库，并修改礼品券状态)|
...
| code       | String    | 200：成功 400：失败 |
| info       | String    | code为200时显示内容为成功信息，为400时显示内容为错误信息 |
| data       | String    | action为"0"时，返回num为用户可抽奖的次数 为"1"时，返回 id：礼券id，mode：礼券编号 为"2"时，返回""|
